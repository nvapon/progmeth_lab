...
import static logic.GameLogic.addLetterToCurrentPlayer;
	...
	public KeyboardPane() {
		...
		for(int i = 0; i < 3; i++) {
			...
			for(String s: keyArray[i]) {
				...
				btnDict.put(s, kB);
				/*
				 * FILL CODE: should call a static method addLetterToCurrentPlayer(String s) from the class GameLogic.
				 */
                kB.setOnMouseClicked(addLetter -> {GameLogic.addLetterToCurrentPlayer(s);} );
